...
\begin{abstract} 
Industry 4.0 has made huge progress by providing exciting new applications facilitating, for example, factory management and configuration. However, less attention has been given on the security concerns introduced by the greater device connectivity. This is particularly bothersome with the use of technologies, such as, Internet of Things (IoT) which increases the attack surface of the system. These vulnerabilites have been witnessed by recent attacks involving IoT devices. The goal of this project is to study the use of Model Based Security Engineering techniques in building (more) secure Industry 4.0 systems. We will use the pick-and-place station already available at fortiss as main use-case modeling and analyzing its security concerns in an extension of \autofocus. 
\end{abstract}
...
\paragraph{Opportunity 3:} We expect a closer interaction between the Model-Based Software Engineering and Industry 4.0 competence fields. \textbf{This will lead to further collaborations between these (complementary) groups.}
